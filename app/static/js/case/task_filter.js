export default {
	...
	setup(props, {emit}) {
		...
		async function asc_desc_filter(change=false){
			...
			if (current_filter){
				if(show_ongoing)
					res = await fetch('/case/'+props.cases_info.case.id+'/ongoing?filter=' + current_filter)
				else
					res = await fetch('/case/'+props.cases_info.case.id+'/finished?filter=' + current_filter)
				let loc = await res.json()
				if(asc_desc)
					emit('tasks_list', loc)
				else
