...
![Flowintel system architecture](technical-specifications-diagrams/flowintel-technical-Architecture.png)

```mermaid
flowchart TB
    browser["Web browser / REST client"]
    nginx["NGINX: TLS, static files, reverse proxy"]

    subgraph app["Flowintel application (Gunicorn / Flask, N workers)"]
        direction TB
        pres["Presentation and API layer"]
        core["Business-logic layer (Core services)"]
        data["Data-access layer (SQLAlchemy models)"]
        pres --> core
        core --> data
    end
...
| **Reverse proxy** | **NGINX** | TLS termination, static file delivery, request proxying; `ProxyFix` reads `X-Forwarded-*` |
...
**Dependency management.** The Python dependencies are kept in two files. `requirements.in`
is the short source list that the developers maintain by hand, and it names only the direct
dependencies. `requirements.txt` is generated from `requirements.in` with `pip-compile`
(part of pip-tools). It pins every package to an exact version and adds the indirect
(transitive) dependencies, so every install uses the same tested set. A few dependencies are
pinned to a specific Git commit instead of a released version.
...
```mermaid
flowchart TB
    subgraph client_node["Client device"]
        ua["Web browser / API client"]
    end

    subgraph server["Application server (Ubuntu LTS)"]
        nginx["«service» NGINX<br/>:443 TLS"]
        subgraph gunicorn["«service» Gunicorn"]
            w1["Flask worker 1"]
            w2["Flask worker 2"]
            wn["Flask worker N"]
        end
        pg["«service» PostgreSQL<br/>:5432"]
        vk["«service» Valkey<br/>:6379"]
        mm["«service» misp-modules<br/>:6666"]
        notif["«service» notifications worker"]
        fs[["uploads/, logs/, template repos"]]
    end

    subgraph ext["External infrastructure (optional)"]
        misp["MISP instance"]
...
| UC-18 | Central repository of templates | Template Editor, Admin | Register template repositories and import case and task templates from them. | Manage repositories; add or compare a template; invalid template; permission denied |
...
| UC-25 | Manage notifications | Any user | View and filter the notifications about items assigned to the user. | Mark read or unread; mark all read; delete |
...
| UC-28 | Export case report | Editor, Admin | Export a case report as JSON, Markdown or PDF. | Download; save to the case; attach to MISP; missing data |
...
**Example, in the outline format: UC-11 Manage users, create-a-user flow**
...
sequenceDiagram
    ...
    NGINX->>GU: proxy HTTP (X-Forwarded-*)
    GU->>V: dispatch route
