...
function verify_all {
    ...
    if [ -f "$SCRIPT_DIR/uv.lock" ]; then
        print_status "uv.lock trouvé"
    else
        print_warning "uv.lock absent (sera généré)"
    fi
    ...
    if [ -f "$SCRIPT_DIR/app/assets/package.json" ]; then
        ...
        elif [ -f "$SCRIPT_DIR/app/assets/package-lock.json" ]; then
            print_warning "package-lock.json trouvé (migration vers bun.lock recommandée)"
            warnings=$((warnings + 1))
        else
            print_warning "Aucun lock file Node.js"
            warnings=$((warnings + 1))
        fi
    fi
    ...
    if command -v valkey-server &> /dev/null; then
        ...
        print_warning "Valkey/Redis non trouvé (optionnel si Docker)"
        warnings=$((warnings + 1))
    fi
...
function migrate {
    ...
    if [ -f "package.json" ] && [ -f "package-lock.json" ]; then
        if [ ! -f "bun.lock" ]; then
            ...
            print_status "bun.lock généré depuis package.json"
        fi
    ...
    print_info "Mise à jour de pyproject.toml..."
    if [ -f "pyproject.toml" ]; then
        if ! grep -q "\[project\]" pyproject.toml; then
            cat > pyproject.toml << 'EOF'
# pyproject.toml - Flowintel
[project]
name = "flowintel"
version = "3.1.0"
description = "Open-source case management platform for security analysts"
...
EOF
            print_status "pyproject.toml mis à jour pour uv"
        else
            print_status "pyproject.toml déjà à jour"
        fi
    fi
...
function delete_temp_files {
    ...
    if [ -f "$SCRIPT_DIR/app/assets/yarn.lock" ]; then
        rm "$SCRIPT_DIR/app/assets/yarn.lock"
        print_status "yarn.lock supprimé"
        deleted=$((deleted + 1))
    fi

    if [ -d "$SCRIPT_DIR/app/assets/node_modules" ]; then
        print_warning "node_modules trouvé (peut être regeneré avec bun install)"
    fi
