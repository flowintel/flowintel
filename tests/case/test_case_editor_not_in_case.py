...
def test_edit_case(client):
    ...
    response = client.get("/api/case/1", headers={"X-API-KEY": API_KEY})
    assert response.status_code == 200 and b"Test edit Case editor" not in response.data
...
def test_edit_task(client):
    ...
    response = client.get("/api/case/1/task/1", headers={"X-API-KEY": API_KEY})
    assert response.status_code == 200 and b"Test edit task editor" not in response.data
