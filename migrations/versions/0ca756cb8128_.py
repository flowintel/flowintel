...
def upgrade():
	...
	if 'nb_tasks' not in case_columns:
		op.add_column('case', sa.Column('nb_tasks', sa.Integer(), nullable=True))
		op.create_index('ix_case_nb_tasks', 'case', ['nb_tasks'], unique=False)

	# Check task table
	task_columns = [col['name'] for col in inspector.get_columns('task')]
	if 'case_order_id' not in task_columns:
		op.add_column('task', sa.Column('case_order_id', sa.Integer(), nullable=True))
		op.create_index('ix_task_case_order_id', 'task', ['case_order_id'], unique=False)
